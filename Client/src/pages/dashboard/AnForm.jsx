...
const AnForm = ({ close, item }) => {
  const [name, setName] = useState(item === "" ? "" : item.text);
  const [assign, setAssign] = useState(
    item === "" ? "Harshita" : item.createdBy
  );
  ...
  const onSubmit = () => {
    ...
    if (name !== "") {
      ...
      dispatch(postAnnouncement(body));
      close();
    } else {
    ...
    >
      <div class="relative top-20 mx-auto p-10 border w-2/5 shadow-2xl rounded-xl bg-white flex flex-col items-center justify-center">
        ...
        <div class="flex flex-col items-start justify-start w-full my-2">
          <label class="text-sm py-2">Text:</label>
          <input
        ...
        <div class="flex flex-col items-start justify-start w-full my-2">
          <label class="text-sm py-2">Created By:</label>
          <select
            value={assign}
        ...
        <div class="flex flex-row items-center justify-between mt-8">
          ...
          >
            {item === "" ? "Add Announcement" : "Edit Announcement"}
          </button>
        </div>
      </div>
    </div>
